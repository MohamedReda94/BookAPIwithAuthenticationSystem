const { default: book } = require('../Models/book.cjs')
...
exports.getOneBook = async function(req,res){
    try{
        ...
        if (Book.length ===0){
            return res.json({"message":"Book Not find ", data:[]})
        }else{
...
exports.addNewBook = async function(req,res){
    try{
        ...
        return res.json({message:"done" , data:createdBook})

        } else {
            return res.status(403).send({message:"You dont have the right permission"})
        }
